"""
Script: scripts/run_real_fedavg_dp_d2_1_experiment.py
...
Consumes privatized gradients via SGD with Momentum (momentum = 0.9, lr = 1e-3, weight_decay = 1e-5)
to test calibrated learning-rate scaling for private SGD.
Strictly isolates validation (202 subjects) and maintains the test firewall (204 subjects locked, zero access).
All artifacts are strictly isolated under the 'fedavg_dp_d2_1' namespace.
"""
...
logger = logging.getLogger("fedavg_dp_d2_1_experiment")
...
def train_hospital_silo_dp(
    ...
    """
    Executes local DP-SGD training on a single hospital silo using SGD with Momentum (momentum=0.9, lr=1e-3).
    ...
    # CAUSAL CHANGE: lr = 1e-3 (was 1e-4 in D2)
...
def run_fedavg_dp_d2_1_experiment(
    config_path: Path = PROJECT_ROOT / "configs" / "experiments" / "real_brats_dp_d2_1.yaml",
    ...
    logger.info("=" * 80)
    logger.info("🔒 FEDMED OS — EXPERIMENT D2.1: SGD WITH MOMENTUM DP-SGD (lr = 1e-3, C = 0.06, Momentum = 0.9)")
    logger.info("=" * 80)
    ...
    # 5. Checkpoint Directory Setup (Isolated D2.1 paths)
    if ckpt_dir is None:
        ckpt_dir = PROJECT_ROOT / "checkpoints" / "fedavg_dp_d2_1"
    ckpt_dir.mkdir(parents=True, exist_ok=True)
    best_ckpt_path = ckpt_dir / "best.pt"
    latest_ckpt_path = ckpt_dir / "latest.pt"

    if report_path is None:
        report_path = PROJECT_ROOT / "reports" / "real_brats2024" / "fedavg_dp_d2_1.json"
    if history_path is None:
        history_path = PROJECT_ROOT / "reports" / "real_brats2024" / "fedavg_dp_d2_1_history.json"
    if progress_path is None:
        progress_path = PROJECT_ROOT / "reports" / "real_brats2024" / "fedavg_dp_d2_1_progress.json"
    if manifest_path is None:
        manifest_path = PROJECT_ROOT / "reports" / "real_brats2024" / "dp_d2_1_experiment_manifest.json"
    ...
    # Resume capability strictly within D2.1 checkpoint
    if resume and latest_ckpt_path.exists():
        logger.info(f"Resuming from latest D2.1 checkpoint: {latest_ckpt_path}")
    ...
    logger.info(f"Beginning FedAvg+DP D2.1 Experiment for {rounds} rounds across 4 hospitals (SGD+Momentum, lr={t_cfg.get('learning_rate', 1e-3)}, C={p_cfg.get('max_grad_norm')})...")
    round_durations = []
    strategy = FedAvg(min_fit_clients=4, min_available_clients=4)

    for r in range(start_round, rounds + 1):
        round_start_time = time.perf_counter()
        logger.info(f"\n{'='*70}\n🔄 Starting Federated Round {r}/{rounds} (FedAvg + DP-SGD D2.1, lr={t_cfg.get('learning_rate')}, C={p_cfg.get('max_grad_norm')})\n{'='*70}")
        ...
        for h_name in hospitals:
            logger.info(f"[{h_name}] Executing local DP-SGD training on {len(hospital_dirs[h_name])} subjects (lr={t_cfg.get('learning_rate')}, C={p_cfg.get('max_grad_norm')})...")
    ...
    manifest_record = {
        ...
        "causal_change": "learning_rate 1e-4 -> 1e-3 (SGD with Momentum=0.9, C=0.06, sigma=0.87)",
        ...
        "invariant_parameters": {
            ...
            "optimizer": f"SGD(lr={t_cfg.get('learning_rate', 1e-3)}, momentum={t_cfg.get('momentum', 0.9)}, weight_decay={t_cfg.get('weight_decay', 1e-5)})",
            "batch_size": 1,
            "local_epochs": 1,
            "rounds": rounds,
    ...
    logger.info("=" * 80)
    logger.info("🎉 EXPERIMENT D2.1 EXECUTION FINISHED")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run Real-Data FedAvg + SGD with Momentum DP-SGD (Experiment D2.1, lr=1e-3)")
    parser.add_argument("--config", type=str, default=str(PROJECT_ROOT / "configs" / "experiments" / "real_brats_dp_d2_1.yaml"))
