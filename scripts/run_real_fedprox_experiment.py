"""
Script: scripts/run_real_fedprox_experiment.py

Purpose:
FEDMED OS — PHASE 10 / EXPERIMENT C: REAL-DATA FEDERATED LEARNING (FedProx).
Executes proximal-regularized Federated Optimization (FedProx) across 4 hospital silos
(hospital_alpha, hospital_beta, hospital_gamma, hospital_delta) on the BraTS-GLI 2024 cohort.
Applies proximal term (mu/2) * ||w - w_global||^2 to constrain local client drift under non-IID partitions.
...
logger = logging.getLogger("fedprox_real_experiment")
...
def evaluate_global_model(
    ...
    Uses unbiased standard segmentation loss (excluding proximal penalty).
    """
    model.eval()
    val_losses = []
...
def train_hospital_silo_fedprox(
    hospital_name: str,
...
) -> Tuple[Dict[str, torch.Tensor], float, float, float, int, float]:
    """
    Executes local training on a single hospital silo using FedProx objective:
    L_prox(w) = L_base(w) + (mu/2) * ||w - w_global||^2
    Returns updated local weights, base loss, proximal penalty, total loss, sample count, and duration.
    """
    ...
    base_loss_fn = DiceCELoss(
        sigmoid=loss_params.get("sigmoid", True),
        lambda_dice=loss_params.get("lambda_dice", 1.0),
        lambda_ce=loss_params.get("lambda_ce", 0.2),
    )

    # Wrap with FedProx criterion and set frozen global parameter reference
    fedprox_criterion = FedProxCriterion(base_loss_fn=base_loss_fn, mu=mu)
    global_params_frozen = [v.to(device).clone().detach() for v in global_state_dict.values()]
    fedprox_criterion.set_global_parameters(global_params_frozen)
    ...
    trainable_params = list(local_model.parameters())
    ...
    for batch in loader:
        ...
        t_loss, b_loss, p_penalty = fedprox_criterion(logits, targets, trainable_params)

        if torch.isnan(t_loss) or torch.isinf(t_loss):
            raise ValueError(f"Numerical instability in {hospital_name} FedProx training")

        t_loss.backward()
        torch.nn.utils.clip_grad_norm_(local_model.parameters(), max_norm=100.0)
        optimizer.step()

        total_losses.append(t_loss.item())
        base_losses.append(b_loss.item())
        prox_penalties.append(p_penalty.item())
        step_count += 1
    ...
    mean_base_loss = float(np.mean(base_losses)) if base_losses else 0.0
    mean_prox_penalty = float(np.mean(prox_penalties)) if prox_penalties else 0.0

    local_weights = {k: v.cpu().clone().detach() for k, v in local_model.state_dict().items()}

    return local_weights, mean_base_loss, mean_prox_penalty, mean_total_loss, len(subject_dirs), duration


def run_fedprox_experiment(
    config_path: Path = PROJECT_ROOT / "configs" / "experiments" / "real_brats_fedprox.yaml",
    ...
    logger.info("=" * 80)
    logger.info("🌐 FEDMED OS — EXPERIMENT C: REAL-DATA FEDERATED LEARNING (FedProx)")
    logger.info("=" * 80)
    ...
    proximal_mu = float(mu if mu is not None else fed_cfg.get("proximal_mu", 0.01))
    ...
    logger.info(f"Execution Device: {device} | Proximal Mu (μ): {proximal_mu}")
    ...
    # Track individual peak rounds
    peak_rounds = {
        "wt_dice": {"peak_value": -1.0, "round": -1},
        "tc_dice": {"peak_value": -1.0, "round": -1},
        "et_dice": {"peak_value": -1.0, "round": -1},
        "macro_dice": {"peak_value": -1.0, "round": -1},
    }
    ...
    if resume and latest_ckpt_path.exists():
        ...
        peak_rounds = checkpoint.get("peak_rounds", peak_rounds)
        logger.info(f"Resumed at round {start_round}, prior best macro dice: {best_mean_dice:.4f}")

    # 6. Federated Rounds Loop
    logger.info(f"Beginning FedProx Experiment for {rounds} rounds (mu={proximal_mu}) across 4 hospitals...")
    round_durations = []
    strategy = FedProx(proximal_mu=proximal_mu, min_fit_clients=4, min_available_clients=4)

    for r in range(start_round, rounds + 1):
        round_start_time = time.perf_counter()
        logger.info(f"\n{'='*70}\n🔄 Starting Federated Round {r}/{rounds} (FedProx μ={proximal_mu})\n{'='*70}")
        ...
        for h_name in hospitals:
            logger.info(f"[{h_name}] Executing local FedProx training round on {len(hospital_dirs[h_name])} subjects...")
            loc_weights, loc_base_loss, loc_prox_penalty, loc_total_loss, n_samples, loc_time = train_hospital_silo_fedprox(
            ...
            client_base_losses[h_name] = loc_base_loss
            client_prox_penalties[h_name] = loc_prox_penalty
            client_total_losses[h_name] = loc_total_loss
            ...
            fit_results.append(FitResult(
                ...
                metrics={
                    "training_loss": loc_total_loss,
                    "base_loss": loc_base_loss,
                    "proximal_penalty": loc_prox_penalty,
                    "hospital": h_name,
                },
            ))
            logger.info(
                f"[{h_name}] Training Complete in {loc_time:.1f}s | "
                f"Base Loss: {loc_base_loss:.6f} | Prox Penalty: {loc_prox_penalty:.6f} | Total Loss: {loc_total_loss:.6f}"
            )
        ...
        # Sample-Weighted FedProx Aggregation via Server Strategy
        ...
        logger.info(f"FedProx Aggregation Complete | Global Delta ||W_new - W_old|| = {delta_global:.6f}")

        # Global Evaluation on Held-Out 202 Validation Cohort (Unbiased Base Loss)
        ...
        # Update peak metrics
        if val_metrics["wt_dice"] > peak_rounds["wt_dice"]["peak_value"]:
            peak_rounds["wt_dice"] = {"peak_value": val_metrics["wt_dice"], "round": r}
        if val_metrics["tc_dice"] > peak_rounds["tc_dice"]["peak_value"]:
            peak_rounds["tc_dice"] = {"peak_value": val_metrics["tc_dice"], "round": r}
        if val_metrics["et_dice"] > peak_rounds["et_dice"]["peak_value"]:
            peak_rounds["et_dice"] = {"peak_value": val_metrics["et_dice"], "round": r}
        if val_metrics["mean_dice"] > peak_rounds["macro_dice"]["peak_value"]:
            peak_rounds["macro_dice"] = {"peak_value": val_metrics["mean_dice"], "round": r}

        logger.info(
            f"Round {r:02d} Summary: Mean Client Total Loss={agg_metrics['training_loss']:.6f} | "
            ...
            f"Macro Dice={val_metrics['mean_dice']:.4f} | Round Time={round_total_duration:.1f}s"
        )

        round_record = {
            "round": r,
            "mean_client_total_loss": round(agg_metrics["training_loss"], 6),
            "mean_client_base_loss": round(float(np.mean(list(client_base_losses.values()))), 6),
            "mean_client_prox_penalty": round(float(np.mean(list(client_prox_penalties.values()))), 6),
            "client_losses": {k: round(v, 6) for k, v in client_total_losses.items()},
            "client_base_losses": {k: round(v, 6) for k, v in client_base_losses.items()},
            "client_prox_penalties": {k: round(v, 6) for k, v in client_prox_penalties.items()},
        ...
        # Primary model selection: Macro Dice
    ...
    # 7. Compile Final Manifest & Report
    ...
    manifest_record = {
        ...
        "random_seed": seed,
        "invariant_parameters_with_experiment_b": {
        ...
        "changed_parameter_from_experiment_b": {
            "parameter": "strategy",
            "experiment_b_value": "FedAvg",
            "experiment_c_value": "FedProx",
            "proximal_mu": proximal_mu,
            "objective_function": "L_prox(w) = L_base(w) + (mu/2) * ||w - w_global||^2",
        },
    ...
    final_report = {
        ...
        "performance_summary": {
            ...
            "peak_rounds": peak_rounds,
            "final_train_loss": round_history[-1]["mean_client_total_loss"] if round_history else None,
    ...
    report_path = PROJECT_ROOT / "reports" / "real_brats2024" / "fedprox_real.json"
    report_path.parent.mkdir(parents=True, exist_ok=True)
    with open(report_path, "w") as f:
        json.dump(final_report, f, indent=2)

    history_path = PROJECT_ROOT / "reports" / "real_brats2024" / "fedprox_real_history.json"
    with open(history_path, "w") as f:
        json.dump(round_history, f, indent=2)

    logger.info("=" * 80)
    logger.info("🎉 EXPERIMENT C COMPLETE: REAL-DATA FEDPROX")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run Real-Data FedProx (Experiment C)")
    parser.add_argument("--rounds", type=int, default=20, help="Number of federated rounds")
    parser.add_argument("--mu", type=float, default=None, help="Proximal regularization mu")
    ...
    run_fedprox_experiment(rounds=args.rounds, mu=args.mu, max_steps_per_client=args.max_steps, resume=not args.no_resume)
