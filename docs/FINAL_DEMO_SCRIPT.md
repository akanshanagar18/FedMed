...
| **8** | Point to the Tumor Volume metrics | *"The engine quantifies exact anatomical tumor sub-regions: Whole Tumor, Tumor Core, and Enhancing Tumor volumes in cubic millimeters with strict hierarchical enclosure."* | Three metric cards showing Whole Tumor ($1,199,335.6\text{ mm}^3$), Tumor Core ($1,194,166.7\text{ mm}^3$), and Enhancing Tumor ($728,934.3\text{ mm}^3$), strictly satisfying $\text{ET} \le \text{TC} \le \text{WT}$. |
