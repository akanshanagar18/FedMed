"""
Module: server.strategies.fedadam

Purpose:
FedAdam (Adaptive Server Optimization for Federated Learning, Reddi et al., ICLR 2021).
Applies Adam adaptive momentum (m_t) and second moment (v_t) to aggregated client pseudogradient updates.
"""
...
logger = get_logger("fedadam")


@register_strategy("FedAdam")
class FedAdam(BaseStrategy):
    """
    FedAdam strategy implementing server-side Adam adaptive optimization.
    """
    ...
    def get_metadata(self) -> StrategyMetadata:
        return StrategyMetadata(
            name="FedAdam",
            version="1.0.0",
            description="Adaptive Federated Optimization with Server Adam (Reddi et al., ICLR 2021)",
            supported_features=["server_momentum", "adaptive_learning_rate", "differential_privacy", "homomorphic_encryption"],
    ...
    def _init_moments(self, params: List[np.ndarray]) -> None:
        if self.m_t is None:
            ...
            self.v_t = [np.zeros_like(p, dtype=np.float32) for p in params]
    ...
    def aggregate_fit(
        ...
        if not results or len(results) < self.min_fit_clients:
            logger.warning(f"[FedAdam] Insufficient fit results ({len(results)}/{self.min_fit_clients})")
            return None, {}

        # 1. Standard FedAvg weighted pseudogradient target: w_target
        ...
        # 3. Update server 1st & 2nd moments
        for i in range(len(delta)):
            self.m_t[i] = self.beta_1 * self.m_t[i] + (1.0 - self.beta_1) * delta[i]
            self.v_t[i] = self.beta_2 * self.v_t[i] + (1.0 - self.beta_2) * (delta[i] ** 2)
        ...
        logger.info(f"[FedAdam] Round {server_round} - Loss: {avg_loss:.4f}, Dice: {avg_dice:.4f}, m_norm: {m_norm:.4f}")
