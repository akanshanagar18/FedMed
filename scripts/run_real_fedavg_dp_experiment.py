"""
Script: scripts/run_real_fedavg_dp_experiment.py

Purpose:
FEDMED OS — PHASE 10 / EXPERIMENT D: REAL-DATA FEDERATED DIFFERENTIAL PRIVACY (FedAvg + DP-SGD).
...
Applies per-sample gradient norm clipping (C = 1.0) and calibrated Gaussian noise (sigma = 0.5)
...
logger = logging.getLogger("fedavg_dp_real_experiment")
...
def train_hospital_silo_dp(
    ...
    """
    Executes local DP-SGD training on a single hospital silo using true Poisson / Bernoulli subsampling.
    Performs per-sample gradient clipping to max_grad_norm and calibrated Gaussian noise injection.
    """
    ...
    max_grad_norm = p_cfg.get("max_grad_norm", 1.0)
...
def run_fedavg_dp_experiment(
    config_path: Path = PROJECT_ROOT / "configs" / "experiments" / "real_brats_dp.yaml",
    ...
    logger.info("=" * 80)
    logger.info("🔒 FEDMED OS — EXPERIMENT D: REAL-DATA FEDERATED LEARNING (FedAvg + DP-SGD)")
    logger.info("=" * 80)
    ...
    logger.info(f"Beginning FedAvg+DP Experiment for {rounds} rounds across 4 hospitals...")
    round_durations = []
    strategy = FedAvg(min_fit_clients=4, min_available_clients=4)

    for r in range(start_round, rounds + 1):
        round_start_time = time.perf_counter()
        logger.info(f"\n{'='*70}\n🔄 Starting Federated Round {r}/{rounds} (FedAvg + DP-SGD)\n{'='*70}")
        ...
        for h_name in hospitals:
            logger.info(f"[{h_name}] Executing local DP-SGD training on {len(hospital_dirs[h_name])} subjects...")
        ...
        round_record = {
            ...
            "privacy_telemetry": {
                "max_grad_norm_C": p_cfg.get("max_grad_norm", 1.0),
        ...
        history_path = PROJECT_ROOT / "reports" / "real_brats2024" / "fedavg_dp_real_history.json"
        history_path.parent.mkdir(parents=True, exist_ok=True)
        with open(history_path, "w") as f:
            json.dump(round_history, f, indent=2)

        # Continuously update machine-readable live progress file
        ...
        progress_path = PROJECT_ROOT / "reports" / "real_brats2024" / "fedavg_dp_progress.json"
        progress_record = {
    ...
    report_path = PROJECT_ROOT / "reports" / "real_brats2024" / "fedavg_dp_real.json"
    ...
    logger.info("=" * 80)
    logger.info("🎉 EXPERIMENT D COMPLETE: REAL-DATA FEDAVG + DP-SGD")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run Real-Data FedAvg + DP (Experiment D)")
    ...
    run_fedavg_dp_experiment(rounds=args.rounds, max_steps_per_client=args.max_steps, resume=not args.no_resume)
