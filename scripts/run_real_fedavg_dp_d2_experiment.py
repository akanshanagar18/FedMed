"""
Script: scripts/run_real_fedavg_dp_d2_experiment.py

Purpose:
FEDMED OS — PHASE 10.10 / EXPERIMENT D2: REAL-DATA FEDAVG + SGD WITH MOMENTUM DP-SGD (C = 0.06, Momentum = 0.9).
...
Consumes privatized gradients via SGD with Momentum (momentum = 0.9, lr = 1e-4, weight_decay = 1e-5)
to test the hypothesized Adam noise-normalization bottleneck.
Strictly isolates validation (202 subjects) and maintains the test firewall (204 subjects locked, zero access).
All artifacts are strictly isolated under the 'fedavg_dp_d2' namespace.
"""
...
logger = logging.getLogger("fedavg_dp_d2_experiment")
...
def train_hospital_silo_dp(
    ...
    """
    Executes local DP-SGD training on a single hospital silo using SGD with Momentum (momentum=0.9).
    Momentum state is instantiated fresh at the start of each hospital training run, persists across
    all 236 steps in the round, and is discarded upon round completion (standard federated SGD momentum lifecycle).
    """
    ...
    # CAUSAL CHANGE: SGD with Momentum (momentum=0.9, lr=1e-4, weight_decay=1e-5) replacing Adam
...
def run_fedavg_dp_d2_experiment(
    config_path: Path = PROJECT_ROOT / "configs" / "experiments" / "real_brats_dp_d2.yaml",
    ...
    logger.info("=" * 80)
    logger.info("🔒 FEDMED OS — EXPERIMENT D2: SGD WITH MOMENTUM DP-SGD (C = 0.06, Momentum = 0.9)")
    logger.info("=" * 80)
    ...
    logger.info(
        f"Execution Device: {device} | Optimizer: {t_cfg.get('optimizer')} (lr={t_cfg.get('learning_rate')}, momentum={t_cfg.get('momentum')}) | "
        f"DP Mechanism: {p_cfg.get('mechanism')} (σ={p_cfg.get('noise_multiplier')}, C={p_cfg.get('max_grad_norm')})"
    )
    ...
    # 5. Checkpoint Directory Setup (Isolated D2 paths)
    if ckpt_dir is None:
        ckpt_dir = PROJECT_ROOT / "checkpoints" / "fedavg_dp_d2"
    ckpt_dir.mkdir(parents=True, exist_ok=True)
    best_ckpt_path = ckpt_dir / "best.pt"
    latest_ckpt_path = ckpt_dir / "latest.pt"

    if report_path is None:
        report_path = PROJECT_ROOT / "reports" / "real_brats2024" / "fedavg_dp_d2.json"
    if history_path is None:
        history_path = PROJECT_ROOT / "reports" / "real_brats2024" / "fedavg_dp_d2_history.json"
    if progress_path is None:
        progress_path = PROJECT_ROOT / "reports" / "real_brats2024" / "fedavg_dp_d2_progress.json"
    if manifest_path is None:
        manifest_path = PROJECT_ROOT / "reports" / "real_brats2024" / "dp_d2_experiment_manifest.json"
    ...
    # Resume capability strictly within D2 checkpoint
    if resume and latest_ckpt_path.exists():
        logger.info(f"Resuming from latest D2 checkpoint: {latest_ckpt_path}")
    ...
    logger.info(f"Beginning FedAvg+DP D2 Experiment for {rounds} rounds across 4 hospitals (SGD+Momentum, C={p_cfg.get('max_grad_norm')})...")
    round_durations = []
    strategy = FedAvg(min_fit_clients=4, min_available_clients=4)

    for r in range(start_round, rounds + 1):
        round_start_time = time.perf_counter()
        logger.info(f"\n{'='*70}\n🔄 Starting Federated Round {r}/{rounds} (FedAvg + DP-SGD D2, SGD+Momentum, C={p_cfg.get('max_grad_norm')})\n{'='*70}")
        ...
        for h_name in hospitals:
            logger.info(f"[{h_name}] Executing local DP-SGD training on {len(hospital_dirs[h_name])} subjects (SGD+Momentum, C={p_cfg.get('max_grad_norm')})...")
    ...
    manifest_record = {
        "experiment_name": "EXPERIMENT_D2_DP_SGD_MOMENTUM",
        "parent_baseline": "EXPERIMENT_D1_CALIBRATED_CLIPPING",
        "causal_change": "optimizer Adam -> SGD with Momentum=0.9 (lr=1e-4, weight_decay=1e-5)",
        ...
        "invariant_parameters": {
            ...
            "optimizer": f"SGD(lr={t_cfg.get('learning_rate', 1e-4)}, momentum={t_cfg.get('momentum', 0.9)}, weight_decay={t_cfg.get('weight_decay', 1e-5)})",
            "batch_size": 1,
            "local_epochs": 1,
            "rounds": rounds,
        ...
        "privacy_parameters": {
            ...
            "max_grad_norm_C": p_cfg.get("max_grad_norm", 0.06),
            "noise_multiplier_sigma": p_cfg.get("noise_multiplier", 0.87),
            "physical_noise_std": round(p_cfg.get("noise_multiplier", 0.87) * p_cfg.get("max_grad_norm", 0.06), 4),
            "target_delta": p_cfg.get("target_delta", 1e-5),
    ...
    logger.info("=" * 80)
    logger.info("🎉 EXPERIMENT D2 EXECUTION FINISHED")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run Real-Data FedAvg + SGD with Momentum DP-SGD (Experiment D2)")
    parser.add_argument("--config", type=str, default=str(PROJECT_ROOT / "configs" / "experiments" / "real_brats_dp_d2.yaml"))
